...
module.exports = function (componentPath, result) {
  ...
  fs.writeFileSync(`${componentPath}-zh.html`, demoTemplate.zh);
  // fs.writeFileSync(path.join(componentPath, `en.html`), demoTemplate.en);
  const demoComponent = generateDemoComponent(result);
  fs.writeFileSync(`${componentPath}-zh.ts`, demoComponent.zh);
  // fs.writeFileSync(path.join(componentPath, `en.component.ts`), demoComponent.en);
  // const demoModule = generateDemoModule(result);
  // fs.writeFileSync(path.join(componentPath, `index.module.ts`), demoModule);
};
...
function generateDemoComponent(content) {
  ...
  zhOutput = zhOutput.replace(/{{componentName}}/g, `${firstUppercase(component)}Zh`);
